...
Proactively ask follow-up questions about edge cases, input/output formats, example files, success criteria, and dependencies. Wait to start writing until the requirements are solid.
...
**Lean into "pushy" descriptions.** Claude tends to *under-trigger* skills — skipping them even when they'd be useful. Combat this by being explicit and generous with trigger contexts. For example, instead of:

```yaml
description: Build a dashboard to display internal data.
```

Write:

```yaml
description: Build a dashboard to display internal data. Use this skill whenever the user mentions dashboards, data visualization, internal metrics, or wants to display any kind of data, even if they don't explicitly ask for a "dashboard".
```
...
**Explain the "why" instead of over-constraining.** LLMs have good theory of mind — when you explain the reasoning behind a rule, the model can generalize and apply it intelligently. If you find yourself writing ALWAYS or NEVER in all caps, or building rigid structures, that's a yellow flag. Reframe by explaining *why* the behavior matters so the model understands the intent, not just the constraint. This produces skills that are more robust across diverse prompts rather than brittle rules that only work for the examples you tested.
